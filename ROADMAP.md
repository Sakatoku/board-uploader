...
最終更新: 2026-06-21（アイテムのリネーム機能を実装）
...
- ✓ **アイテムのリネーム**: カードヘッダーの ✎ ボタン → モーダルで名前変更。PATCH `/api/boards/:boardId/items/:itemId` に `title` を追加（既存の x/y 更新と統合）。移動は既存のドラッグ＆ドロップで対応済み。テキスト内容の編集は「テキストファイルのリッチ編集」へ。
