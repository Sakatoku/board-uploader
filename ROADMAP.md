...
2. **認証・アクセス制御（Phase 2）** — 第1段 **実装済み（2026-06-18）** / 第2段は今後
   - **第1段＝書き込みゲート（実装済み）**: `WRITE_API_KEY`（環境変数、既定 fail-open）で
     書き込み系（create/notes/files/upload-token/attach/PATCH）を `X-API-Key`（直アップロードは
     clientPayload）で保護。読み取りは開放。UI は「編集キー」入力（localStorage）。
     ユニット＋API＋ブラウザで検証済み。**残: 本番で `WRITE_API_KEY` を設定し有効化＋直アップロードの
     clientPayload 認証を実機確認**（要ユーザー操作。fail-open ゆえ未設定なら影響なし）。
   - **第2段（今後）**: JWT(HttpOnly cookie) + QR ワンタイムトークン。あわせて公開 Blob URL を
     `directUrl` で渡さず**関数プロキシ（`blobs.read`）経由に切替**（読み取りも保護する場合）。
     `serveFile` は既に proxy 経路を持つので分岐追加で対応可。
